    ...
    <>
      <section className="hero">
        ...
        <div className="hero-visual">
          <div className="hero-card-grid">
            {HERO_PRODUCTS.map((p) => (<div className="hero-card" key={p.name}><div className="hero-card-img"><div className="placeholder-candle" style={{ background: p.bg }}><IconFlame />{p.label}</div></div><div className="hero-card-body"><h4>{p.name}</h4><p>{p.desc}</p><div className="price">{p.price}</div></div></div>))}
          </div>
        </div>
      </section>
      ...
      <section className="values">
        ...
        <div className="values-grid">
          {VALUES.map((v, i) => (<FadeUp key={v.title} delay={i * 0.1}><div className="value-item"><div className="value-icon">{v.icon}</div><h4>{v.title}</h4><p>{v.desc}</p></div></FadeUp>))}
        </div>
      </section>
    </>
  )
