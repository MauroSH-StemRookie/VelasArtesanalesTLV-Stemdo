...
function formatearDireccion(dir) {
  ...
  const trozos = []
  if (dir.calle) {
    const calleYNumero = [dir.calle, dir.numero].filter(Boolean).join(' ').trim()
  ...
  return (
    <div className="orders-page">
      <div className="admin-header">
        <button className="admin-back" onClick={() => navigate('/')}><IconBack /> Volver a la tienda</button>
        <h2>Mis Pedidos</h2>
      </div>
        ...
        {!loading && !error && pedidos.length > 0 && (
          <div className="orders-list">
            {pedidos.map(function (p) {
              const estadoCls = 'status-' + String(p.estado || 'pendiente').replace('_', '-')
              return (
                <div className="order-card" key={p.id}>
                  ...
                  <div className="order-card-body">
                    <p><strong>Fecha:</strong> {formatearFecha(p.fecha_creacion)}</p>
                    {formatearDireccion(p.direccion) && (
                      <p><strong>Direccion:</strong> {formatearDireccion(p.direccion)}</p>
                    )}
