...
const UsuarioController = {
    ...
    modificarMiPerfil: async(req, res) => {
        try{
            const idUsuario = req.user.id;
            const { nombre, telefono, calle, numero, cp, ciudad, provincia, piso } = req.body;

            //console.log({ idUsuario, nombre, telefono, calle, numero, cp, ciudad, provincia, piso });
            //Modificar perfil de usuario
            const usuarioActualizado = await UsuarioModel.modificarPerfil(idUsuario, nombre, telefono, calle, numero, cp, ciudad, provincia, piso);
