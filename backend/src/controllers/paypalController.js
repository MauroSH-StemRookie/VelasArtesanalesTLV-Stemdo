...
const captureOrder = async (req, res) => {
  ...
  try {
    ...
    const { body: captureData } = await ordersController.captureOrder({
      id: orderID,
    });
    const capture =
      typeof captureData === "string" ? JSON.parse(captureData) : captureData;
    ...
    if (capture.status !== "COMPLETED") {
      ...
      return res
        .status(400)
        .json({ error: "Pago no completado", status: capture.status });
    }
